...
_UUID_REGEX = re.compile(r"[a-f0-9]{8}-?[a-f0-9]{4}-?[a-f0-9]{4}-?[a-f0-9]{4}-?[a-f0-9]{12}", re.I)
...
def filename_from_title(title: str) -> str:
    ...
    results: list[str] = []
    for char in picard_src.replace_non_ascii(title):  # type: ignore
        if char == "&":
            results.extend("and")
...
def fix(text: str) -> str:
    ...
    text = picard_src.unicode_simplify_combinations(text)  # type: ignore
    # text = picard_src.unicode_simplify_accents(text)  # type: ignore
    text = picard_src.unicode_simplify_punctuation(text)  # type: ignore
    text = picard_src.unicode_simplify_compatibility(text)  # type: ignore
    return text
