...
- `Release` (id = MusicBrainz release id) - aggregate root, represents a specific
  edition of an Album
- `Medium` (id = medium number within its Release) - one physical or digital media
  unit within a Release
- `Track` (id = track number / MB track id within its Medium)

**Album Concept**:

- `Album` is a conceptual work (e.g., "Dark Side of the Moon")
- A `Release` belongs to an `Album` and references it via `album_name` and
  `album_id` (MusicBrainz release-group-id)
- `Release` remains the aggregate root; Album is a reference concept, not a separate
  entity
...
- New to cure primitive obsession:
  - `DiscPosition(number, count)` - validates `1 ≤ number ≤ count` once, replaces
    `"x/y"` string
  - `TrackNumber(int)` - knows how to render as `"02"` and parse from filename
  - `AudioFormat` - FLAC/M4A/MP3 as first-class concept
- Implementation: Use `@attr.frozen=True` for immutability and ergonomics (attrs is
...
- Root enforces invariants: track numbers unique/contiguous, source file count matches
  track count

**Domain Services**:

- Logic that doesn't naturally belong to one entity (e.g., matching ripped files to
  tracks)
...
- **`LibraryRepository`**
  - Methods: `store(release, media, files)`, `path_for(release)`
  - Adapters: `FilesystemLibraryRepository`
  - Source today: `Base._move_files`, `get_artist_album_path`
...
- **Library** — the organized, tagged destination (the `flac`/`m4a`/`mp3`/`source`
  trees). *Repository.*
- **Manifest** — the persisted provenance of a `Release` in the `Library`, enabling
  *Reconvert*.
...
- **After Phase 0**: Why is the dependency rule important? What happens if it's
  violated?
- **After Phase 1**: Which of my model classes are entities and which are value
