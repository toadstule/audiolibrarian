...
We need to capture important architectural decisions in the audiolibrarian project.
As the codebase evolves and potentially moves toward Domain-Driven Design, we need a
way to document significant architectural choices, their rationale, and their
consequences.

Without a formal process for recording these decisions:

- Knowledge about why certain architectural choices were made may be lost over
  time
...
- Architectural drift may occur without clear documentation of the intended
  direction

## Decision

We will adopt Architecture Decision Records (ADRs) as defined by the
architecture-decision-record project
...
- When a decision is superseded, the status is changed to "Superseded" and a link
  to the new decision is added
...
- ADRs can be updated to reflect new information if the core decision remains
  valid
